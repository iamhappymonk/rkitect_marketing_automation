...
```bash
ls brand_alchemy/icp.md 2>/dev/null && echo "icp found" || echo "icp missing"
ls brand_alchemy/accounts/ 2>/dev/null && echo "accounts found" || echo "accounts missing"
```

**If brand_alchemy/icp.md exists:** Read it. Extract:
...
**If brand_alchemy/accounts/{company}.md exists:** Read the relevant file. Extract:
...
**Save to file:**
```bash
mkdir -p brand_alchemy/sequences
cat > brand_alchemy/sequences/SIGNAL-TYPE.md << 'EOF'
SEQUENCE_CONTENT_HERE
EOF
echo "Sequence saved to brand_alchemy/sequences/SIGNAL-TYPE.md"
```
