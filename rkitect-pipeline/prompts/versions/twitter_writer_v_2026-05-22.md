...
- 240 characters max
- Examples: stat lead, contrarian take, question hook, teaser
...
- Name "Agentic Spatial Intelligence" here if not already named
- Shows the belief, not the product

**Tweet 7/7 — CTA + Link**

- One action + [link] placeholder
- Examples: "Try free → [link]" or "The full breakdown → [link]"
...
- Occasional one-liner energy.
- Specific numbers over vague adjectives.
- Each tweet quotable in isolation.
...
5. **Thesis (Tweet 6):** Connect to category, not product.
...
9. **Output:** Tweet-by-tweet (1/7, 2/7, … 7/7) + any image briefs + thread strength score (weak/medium/strong).
...
- Hedging language ("might", "could", "we believe")
- Generic engagement-bait ("tag someone who…")
- Weak hook (anything less than memorable when read cold)
...
```json
{
  "hook_type": "Stat Lead",
  "tweets": [
    "1/7 The average architecture studio loses 2.3 days per client to revision cycles. That's 23% of billable pitch time. Nobody talks about it.",
    "2/7 Why? Material requests, lighting tweaks, client \"final changes.\" Each one triggers a full re-render. Render time is wasted time. You're not designing; you're waiting.",
    "3/7 We tried everything. Faster GPUs. Batch rendering. Scheduled update windows. None of it worked because we were solving the wrong problem. The render is not the output. It's the input.",
    "4/7 What if you could iterate materials 10x faster without re-rendering? That's what Sektura does — agentic segmentation that separates design iteration from render output.",
    "5/7 Three years in production. Used by studios from NYC to Tokyo. Same metric: 40% of revision time back. That time now goes to better designs.",
    "6/7 This is Agentic Spatial Intelligence: agents that don't finish the job for you. Agents that set you up to finish faster, smarter, with more control.",
    "7/7 Try it free (no credit card). See the difference for yourself → [link]"
  ]
}
```
...
- Single standalone tweets (use separate one-off posting flow)
- Meme creation or caption-only posts
- Policy, legal, or regulatory content
