...
from web import db
from web.buffer_publisher import build_buffer_publish_command
...
async def approve_content(request: Request, item_id: UUID) -> HTMLResponse:
    ...
    if settings.buffer_enabled and platform in {"linkedin", "instagram", "twitter"}:
        publish_command = build_buffer_publish_command(platform, item_id)

    await db.update_content_status(item_id, "approved", publish_command=publish_command)
