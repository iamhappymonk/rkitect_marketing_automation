...
from .chatgpt_image_provider import ChatGPTImageProvider
from .openrouter_image_provider import OpenRouterImageProvider
...
class CarouselOrchestrator:
    ...
    def __init__(
        ...
        *,
        ...
        # Initialize image provider (Imagen, ChatGPT, or OpenRouter)
        if settings.image_provider.lower() == "openrouter":
            self._imagen_client = OpenRouterImageProvider(
                api_key=settings.openrouter_api_key,
                model=settings.openrouter_image_model,
            )
            logging.getLogger(__name__).info(
                "Using OpenRouter image provider (model: %s)",
                settings.openrouter_image_model,
            )
        elif settings.image_provider.lower() == "chatgpt":
            self._imagen_client = ChatGPTImageProvider(
                api_url=settings.chatgpt_image_api_url,
                timeout_ms=settings.chatgpt_image_timeout_ms,
            )
            logging.getLogger(__name__).info(
                "Using ChatGPT image provider: %s (timeout: %dms)",
                settings.chatgpt_image_api_url,
                settings.chatgpt_image_timeout_ms,
            )
        else:
            ...
            logging.getLogger(__name__).info("Using Imagen image provider")
    ...
    def _build_prompt(self) -> tuple[str, str]:
        ...
        # Provide default brand if not set
        if self._brand:
        ...
        # Provide default persona if not set
        if self._persona:
            ...
            variables["persona"] = {
                ...
                "description": "A professional content creator focused on quality and engagement",
            }
    ...
    def _fallback_prompt() -> str:
        ...
        data = _load_image_fallback_prompts()
        return data["fallback_prompt"].strip()
